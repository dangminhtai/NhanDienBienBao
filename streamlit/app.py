...
Biển báo thường có chữ hoặc hình vẽ màu đen/trắng ở lõi, mô hình sẽ hiểu biển báo là mảnh vỡ li ti rời rạc. Do vậy cần vá lỗ hổng. Việc xóa nhiễu (tẩy hạt bụi) để tránh mô hình dự đoán sai. Cuối cùng dùng findContours & boundingRect ở CV2 để phân vùng và xác định khung hình chữ nhật.                    """)
                    ...
                    geo_filtered_img = img_clahe_bgr.copy()
                    ...
                    for cnt in contours:
                        ...
                        if is_size_ok and is_ratio_ok and is_solid_ok:
                            cv2.rectangle(geo_filtered_img, (x, y), (x + w_box, y + h_box), (0, 255, 0), 2) # Đậu -> Xanh lá
                            geo_passed_count += 1
                        else:
                            cv2.rectangle(geo_filtered_img, (x, y), (x + w_box, y + h_box), (0, 0, 255), 2) # Rớt -> Đỏ chót
                            geo_rejected_count += 1

                    c4_1, c4_2 = st.columns(2)
                    with c4_1:
                        st.image(preview_contours_pil, caption=f"1. Toàn bộ {geo_passed_count + geo_rejected_count} ứng viên từ Bước 3", use_container_width=True)
                    with c4_2:
                        st.image(geo_filtered_img, channels="BGR", caption=f"2. Đánh giá: {geo_passed_count} Đạt (Xanh), {geo_rejected_count} Trượt (Đỏ)", use_container_width=True)
