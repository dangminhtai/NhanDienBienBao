...
Biển báo thường có chữ hoặc hình vẽ màu đen/trắng ở lõi, mô hình sẽ hiểu biển báo là mảnh vỡ li ti rời rạc. Do vậy cần vá lỗ hổng. Việc xóa nhiễu (tẩy hạt bụi) để tránh mô hình dự đoán sai. Cuối cùng dùng findContours & boundingRect ở CV2 để phân vùng và xác định khung hình chữ nhật.                    """)
                    ...
                    for cnt in contours:
                        ...
                        if is_size_ok and is_ratio_ok and is_solid_ok:
                            ...
                            cv2.rectangle(geo_rejected_img, (x, y), (x + w_box, y + h_box), (0, 255, 0), 2) # Vẽ cả ứng viên Xanh lên ảnh gốc
                            geo_passed_count += 1
                        else:
                    ...
                    with c4_1:
                        st.image(geo_rejected_img, channels="BGR", caption=f"1. Tổng quát {geo_passed_count + geo_rejected_count} nghi can (🔴 Đỏ: Bị loại do rỗng tuếch | 🟢 Xanh: Đạt)", use_container_width=True)
                    with c4_2:
...
**Bước 4:** Đây là vòng "Phỏng vấn hình dáng". Biển báo phải có hình vuông, tròn hoặc tam giác đều (Tỷ lệ {det_params['min_ratio']} - {det_params['max_ratio']}) và phải là một khối đặc chứ không phải là những đường kẻ uốn lượn hay cành cây (Độ đặc > {det_params['min_solidity']*100:.0f}%). Những ai quá dài, quá dẹt hoặc rỗng tuếch sẽ bị "đánh trượt" ngay lập tức để tiết kiệm sức cho AI ở các vòng sau.
                    ...
                    if det_params['min_laplacian'] > 0:
                        ...
                        for cnt in contours:
                            ...
                            # Chỉ lấy những hộp đã Đậu ở Bước 4
                            is_size_ok = w_box >= det_params['min_size'] and h_box >= det_params['min_size']
                            is_ratio_ok = det_params['min_ratio'] < aspect_ratio < det_params['max_ratio']
                            is_solid_ok = solidity > det_params['min_solidity']
                            
                            if is_size_ok and is_ratio_ok and is_solid_ok:
                                # Trích xuất Vùng ROI (Region Of Interest)
                                ...
                                if laplacian_var < det_params['min_laplacian']:
                                    ...
                                    # LƯU DỮ LIỆU ĐỂ BƯỚC 6 DÙNG TIẾP
    ...
    **Bước 5:** Phép toán **Laplacian** tính toán "độ sắc cạnh" của chùm pixel. Bạn đã đặt tiêu chuẩn Độ nét = **{det_params['min_laplacian']}**. Các đốm ảnh mờ, nhòe (bokeh, phản chiếu nước) không có cạnh sắc sẽ bị loại bỏ để giảm tải cho AI. (Kéo về 0 ở Sidebar để TẮT vòng kiểm tra này nếu ảnh quá rõ).
                        ...
                        st.info("💡 **Bước 5 (Sát hạch Độ nét)** đã bị TẮT (Do bạn đặt Laplacian = 0). Mô hình sẽ giữ cả những biển báo bị nhòe để gửi trực tiếp cho AI.")
                        st.divider()
                        
                        # Trường hợp B5 TẮT, ta vẫn giả lập lấy hết hình B4 chuyền lên B6
                        passed_rois = []
                        for cnt in contours:
                            x, y, w_box, h_box = cv2.boundingRect(cnt)
                            aspect_ratio = w_box / float(h_box)
                    ...
                    if passed_rois:
                        cached_detector = get_cached_detector()
                        svm_passed = []
                        svm_rejected = []
                        ...
                        for roi_dict in passed_rois:
                            ...
                            # Tính HOG và Lấy Ảnh Trực Quân (Visualized)
                            roi_resized = cv2.resize(roi_bgr, (32, 32))
                            roi_gray = cv2.cvtColor(roi_resized, cv2.COLOR_BGR2GRAY)
                            
                            from skimage.feature import hog
                            params = cached_detector.hog_params.copy()
                            params['visualize'] = True
                            hog_feature, hog_image = hog(roi_gray, **params)
                            
                            # Cường điệu hóa ảnh HOG để người hiển thị rõ hơn (tăng độ tương phản)
                            if np.max(hog_image) > 0:
                                hog_image = hog_image / np.max(hog_image)
                            hog_image_disp = (hog_image * 255).astype(np.uint8)
                            
                            # Scale và Predict bằng mô hình SVM
                            feat_scaled = cached_detector.scaler.transform(hog_feature.reshape(1, -1))
                            prediction = cached_detector.model.predict(feat_scaled)[0]
                            confidence = cached_detector.model.decision_function(feat_scaled)[0]
                            ...
                            if prediction == 1:
                                ...
                                cv2.rectangle(svm_rejected_img, (x, y), (x + w_box, y + h_box), (0, 255, 0), 2) # Nền xanh
                        ...
                        # Show Soi cận cảnh AI (Có ảnh X-Quang)
                        if svm_passed or svm_rejected:
                            ...
                            with c_ai_1:
                                if svm_rejected:
                                    st.caption(f"Trượt (Điểm Âm): Khung xương HOG méo mó, lộn xộn, không ra hình")
                                    ...
                                    for i, roi_dict in enumerate(svm_rejected[:5]):
                                        with cols_ra[i]:
                                            ...
                                            # Hiển thị ảnh X-Quang HOG ngay bên dưới
                                            st.image(roi_dict['hog_img'], caption=f"❌ {roi_dict['score']}", use_container_width=True, clamp=True)
                            with c_ai_2:
                                if svm_passed:
                                    st.caption(f"Đạt (Điểm Dương): Khung xương HOG hiện lên rõ ràng cấu trúc viền")
                        ...
                        st.info("""
**Bước 6:** Đây là bài kiểm tra cốt lõi nhất. Giám khảo Trí tuệ Nhân tạo được đào tạo bằng **HOG (Histogram of Oriented Gradients)** để "nhìn" bức ảnh dạng mạng lưới viền cấu trúc hình học phức tạp (thay vì nhìn màu sắc hay độ nét thông thường). Cỗ máy **SVM** liên hệ với tri thức HOG để tính điểm Độ Tự Tin (Confidence). Khi Điểm > 0, AI tin rằng đây là một Cấu trúc Biển Báo. Rác thải tự nhiên thường bị AI bắt thóp ở vòng thi này!
                        """)
                    else:
                        st.warning("⚠️ Không có ứng viên nào lọt qua Bước 5 để mang đi thi tuyển AI.")
                    ...
                    if passed_rois and len(svm_passed) > 0:
                        nms_before_img = img_clahe_bgr.copy()
                        nms_after_img = img_clahe_bgr.copy()
                        
                        boxes = [item['rect'] for item in svm_passed]
                        probs = [item['conf'] for item in svm_passed]
                        
                        for (x, y, w_box, h_box) in boxes:
                            # Vẽ toàn bộ những khung màu vàng chằng chịt lên
                            cv2.rectangle(nms_before_img, (x, y), (x + w_box, y + h_box), (0, 255, 255), 2)
                        
                        # Chạy thuật toán NMS loại bỏ khung thừa
                        final_boxes, keep_indices = cached_detector.nms(boxes, probs, threshold=det_params.get('nms_thresh', 0.3), return_indices=True)
                        
                        for idx in keep_indices:
                            x, y, w_box, h_box = boxes[idx]
                            # Vẽ cái khung chiến thắng duy nhất màu chói
                            cv2.rectangle(nms_after_img, (x, y), (x + w_box, y + h_box), (0, 255, 0), 3)
                        
                        c7_1, c7_2 = st.columns(2)
                        with c7_1:
                            st.image(nms_before_img, channels="BGR", caption=f"1. Trước NMS: {len(boxes)} khung hình chen chúc", use_container_width=True)
                        with c7_2:
                            st.image(nms_after_img, channels="BGR", caption=f"2. Sau NMS: Lọc lại đúng {len(final_boxes)} Vị trí Độc tôn", use_container_width=True)
                            
                        st.info(f"""
**Bước 7:** Rất nhiều trường hợp AI nhận diện chính xác 1 biển báo, nhưng nó lại bị khoanh bởi 3-4 cái khung với các kích thước/góc độ hơi lệch nhau (Do quá trình Quét Đa Kênh gộp lại). Thuật toán **NMS (Trấn áp Phi Cực Đại)** có nhiệm vụ gom nhóm các khung bị đè lên nhau (Mức độ đè lấp IoU > {det_params['nms_thresh']}), tôn vinh khung nào có điểm AI tuyệt đối (Confidence) cao nhất làm "chuẩn mực", và thẳng tay XÓA SẠCH những khung 'ăn theo' để trả lại 1 khu vực tinh tươm duy nhất. Toàn bộ Quy trình Detect kết thúc tại đây!
                        """)
                    else:
                        st.warning("⚠️ Không có biểu báo hợp lệ để dọn dẹp (Không qua được kiểm duyệt AI).")
