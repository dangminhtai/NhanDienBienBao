...
**Bước 4:** Đây là vòng "Phỏng vấn hình dáng". Biển báo phải có hình vuông, tròn hoặc tam giác đều (Tỷ lệ {det_params['min_ratio']} - {det_params['max_ratio']}) và phải là một khối đặc chứ không phải là những đường kẻ uốn lượn hay cành cây (Độ đặc > {det_params['min_solidity']*100:.0f}%). Những ai quá dài, quá dẹt hoặc rỗng tuếch sẽ bị "đánh trượt" ngay lập tức để tiết kiệm sức cho AI ở các vòng sau.
                    ...
                    with c5_1:
                        st.image(focus_rejected_img, channels="BGR", caption=f"1. Khám thị lực ({focus_passed_count + focus_rejected_count} nghi can) -> 🔴 Đỏ: Đánh rớt vì mờ tịt", use_container_width=True)
                    with c5_2:
                        st.image(focus_passed_img, channels="BGR", caption=f"2. {focus_passed_count} Ứng viên Đạt chuẩn (Hình học đúng & Sắc nét)", use_container_width=True)
                        
                    st.info("""
**Bước 5:** Biển báo giao thông thực tế thường được in ấn rất sắc nét và chứa nhiều ký tự rõ ràng. Trong khi đó, các đốm sáng xuyên qua kẽ lá (bokeh), phản chiếu trên mặt đường nước, hoặc ảnh chụp rung tay thường bị nhòe (Blur). Phép toán **Laplacian** đóng vai trò như một vị "bác sĩ đo thị lực", thẳng tay gạch tên các hình chữ nhật bị mờ/nhòe (Điểm nét < 40) trước khi mang đi "hỏi" Giám khảo Trí tuệ Nhân tạo.
                    """)
                    st.divider()
