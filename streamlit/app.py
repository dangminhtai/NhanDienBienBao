...
**Bước 6:** Đây là bài kiểm tra cốt lõi nhất. Giám khảo Trí tuệ Nhân tạo được đào tạo bằng **HOG (Histogram of Oriented Gradients)** để "nhìn" bức ảnh dạng mạng lưới viền cấu trúc hình học phức tạp (thay vì nhìn màu sắc hay độ nét thông thường). Cỗ máy **SVM** liên hệ với tri thức HOG để tính điểm Độ Tự Tin (Confidence). Khi Điểm > 0, AI tin rằng đây là một Cấu trúc Biển Báo. Rác thải tự nhiên thường bị AI bắt thóp ở vòng thi này!
                    ...
                    if passed_rois and len(svm_passed) > 0:
                        ...
                        enable_nms_demo = st.checkbox("🛠️ Bật Mô phỏng Rác giả lập (Chuyên dùng để Thuyết trình/Dạy học NMS)", value=False, help="Khi hệ thống chạy quá hoàn hảo (1 hình chỉ ra 1 khung), tính năng này sẽ cố tình bơm thêm 5 khung rác chệch hướng đè lên nhau vào để bạn có cơ hội biểu diễn thuật toán dọn dẹp của NMS cho khán giả xem.")
                        ...
                        is_simulated = False
                        # Nếu bật tùy chọn Demo và có quá ít khung đè nhau, ta bơm rác để trực quan hóa
                        ...
                        with c7_1:
                            st.image(zoom_before, channels="BGR", caption=f"1. AI khoanh {len(sim_boxes)} vòng đè nhau (Ghi kèm điểm Conf)", use_container_width=True, clamp=True)
                        with c7_2:
                            st.image(zoom_after, channels="BGR", caption=f"2. NMS diệt kẻ yếu bám gót, giữ {len(final_boxes)} VUA độc tôn", use_container_width=True, clamp=True)
                            
                        st.info(f"""
**Bước 7:** Rất nhiều trường hợp AI nhận diện chính xác 1 biển báo, nhưng nó lại khoanh thành rất nhiều cái khung với các kích thước/góc độ hơi lệch nhau đè nát bét bức hình. Thuật toán **NMS (Trấn áp Phi Cực Đại)** có nhiệm vụ gom nhóm các khung dính chùm này (Mức độ đè lấp IoU > {det_params['nms_thresh']}), phân loại xem khung nào dán số điểm **Confidence từ AI cao nhất** để tôn lên làm "chuẩn mực", và thẳng tay XÓA SẠCH những khung 'ăn theo' yếu điểm hăng còn lại. Toàn bộ Quy trình Khoanh vùng (Detection) chính thức kết thúc hoàn hảo tại đây!
                        """)
                    else:
