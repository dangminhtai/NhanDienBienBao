    ...
    **Bước 5:** Phép toán **Laplacian** tính toán "độ sắc cạnh" của chùm pixel. Bạn đã đặt tiêu chuẩn Độ nét = **{det_params['min_laplacian']}**. Các đốm ảnh mờ, nhòe (bokeh, phản chiếu nước) không có cạnh sắc sẽ bị loại bỏ để giảm tải cho AI. (Kéo về 0 ở Sidebar để TẮT vòng kiểm tra này nếu ảnh quá rõ).
                    ...
                    if passed_rois:
                        ...
                        for roi_dict in passed_rois:
                            ...
                            hog_feature = hog(roi_gray, **cached_detector.hog_params)
                            ...
                            if prediction == 1:
                                ...
                                svm_passed.append({'img': roi_rgb, 'score': score_text})
                            else:
                                ...
                                svm_rejected.append({'img': roi_rgb, 'score': score_text})
                                
                        c6_1, c6_2 = st.columns(2)
                        with c6_1:
                            st.image(svm_rejected_img, channels="BGR", caption=f"1. AI Đánh rớt {len(svm_rejected)} ứng viên mạo danh (HOG lỗi)", use_container_width=True)
                        with c6_2:
                            st.image(svm_passed_img, channels="BGR", caption=f"2. {len(svm_passed)} BIỂN BÁO CHÍNH THỨC 🎯", use_container_width=True)
                            
                        # Show Soi cận cảnh AI
                        if svm_passed or svm_rejected:
                            st.write("#### 🔎 Tại tòa án AI: Trí tuệ Nhân tạo phán quyết điều gì?")
                            c_ai_1, c_ai_2 = st.columns(2)
                            with c_ai_1:
                                if svm_rejected:
                                    st.caption(f"AI phán Trượt (Confidence < 0)")
                            ...
                            with c_ai_2:
                                if svm_passed:
                                    st.caption(f"AI phán Đạt chuẩn Biển Báo! (Confidence > 0)")
