...
## Trải nghiệm Người dùng (UX) và Tối ưu hóa Cuộn (Scroll Optimization)
- **Cấu trúc Giải phẫu Độc lập (Decoupled Anatomy)**: Trong một Dashboard phức tạp, việc lồng ghép quá nhiều thông tin vào một khối duy nhất (như lồng Bước 3, 4 vào Bước 2) sẽ gây mệt mỏi cho người dùng khi phải cuộn quá nhiều. Việc tách rời các bước thành các `st.expander` độc lập giúp người dùng chủ động chọn phần mình muốn xem, cải thiện đáng kể tính công thái học (Ergonomics) của ứng dụng.
...
## Tối giản Giao diện (Minimalism in UI)
- **Sức mạnh của Sự tối giản (Less is More)**: Trong các dashboard kỹ thuật phức tạp, việc nhồi nhét quá nhiều văn bản giải thích có thể phản tác dụng, gây "nhiễu" thị giác. Việc loại bỏ các tiêu đề thừa và chỉ giữ lại Sơ đồ (Diagram) giúp người dùng tập trung hoàn toàn vào luồng xử lý (Data Flow).
- **Trực quan hóa thay thế Văn bản**: Một sơ đồ tốt có giá trị hơn ngàn lời nói. Khi sơ đồ đã tự giải thích được quy trình (ví dụ: CNN ➡️ SVM), các nhãn dán lặp lại trở nên dư thừa và cần được lược bỏ để tối ưu hóa không gian hiển thị.
- **Phản hồi nhanh (Agile Feedback)**: Việc người dùng yêu cầu xóa văn bản ngay khi thấy nó "thừa" cho thấy tầm quan trọng của việc tách biệt nội dung (content.json) để có thể chỉnh sửa cấu trúc thông tin một cách tích thì mà không ảnh hưởng đến logic hệ thống.
- **Quản lý Văn bản Dự phòng (Fallback Strategy)**: Khi thiết kế hệ thống có CMS hoặc I18n, các chuỗi ký tự "dự phòng" (fallback) trong mã nguồn Python cũng cần được quản lý chặt chẽ. Nếu người dùng muốn xóa hoàn toàn văn bản, việc chỉ sửa file cấu hình là chưa đủ nếu mã nguồn vẫn còn các chuỗi mặc định. Cần thiết kế logic hiển thị linh hoạt (như `if text: st.write(text)`) để đảm bảo UI sạch sẽ 100% theo ý muốn.
- **Tính nhất quán trong Đặt tiêu đề (UI Consistency)**: Khi thay đổi tiêu đề trong hệ thống (như chuyển từ tên kỹ thuật "ndaarray" sang tên phổ thông "Tiền xử lý"), cần thực hiện đồng bộ tại cả 4 nơi: file cấu hình (`content.json`), mã nguồn Python (fallback text), tài liệu nhúng (`.md`) và báo cáo (`requirements.md`). Điều này đảm bảo trải nghiệm người dùng không bị đứt gãy và hệ thống luôn chuyên nghiệp.
